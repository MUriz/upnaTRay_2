...
public class PhongMaterial extends Material {
    ...
    public PhongMaterial(final Color ka, final Color ks, final Color kd, final GlossyReflectance Fs, final boolean especular) {
    ...
    private Color directShader(final Group G, final Point3D P, final Vector3D normal, final Point3D V, final Light L) {
        ...
        PV.normalize();
        float mult = this.getFs().reflectance(P, normal, PV, L.getLocation())/normal.dotProduct(I);
        Color p_ks = multColor(this.getKs(), (mult < 0) ? 0 : mult);
        Color retColor = addColors(this.getKd(), p_ks);
        return multColor(retColor, L.getIrradiance(G, P, normal));
